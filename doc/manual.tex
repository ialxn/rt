...
All sources define the keywords {\tt origin} (doubles), {\tt power} (double), {\tt n\_rays} (unsigned integer), and {\tt spectrum} (string) i.e.\ the origin of the source, the total power assigned to the source, the number of rays that start from it, and the name of a file\footnote{The spectrum will be represented by a linear interpolation of the data given in this file. Use a minimum number of data points to make the interpolation fast} containing the spectrum of the source. Each ray thus represents a power of {\tt power}/{\tt n\_rays} at a random wavelength.
...
{\tt get\_flux} extracts the binary output datafiles producted by {\tt rt} and lists the data. It outputs\footnote{tab separated} $x$ $y$ power $\lambda$ $n_{\mathrm{refl}}$ for planar targets or $x$ $y$ $z$ power $\lambda$ $n_{\mathrm{refl}}$ otherwise.
